...
export function WatchlistButton({ symbol, className = '' }: { symbol: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold border transition-colors min-h-11 ${className} ${inList ? 'bg-live/15 border-live/30 text-[#9BE8FF]' : 'border-white/10 text-[var(--muted)] hover:text-white hover:border-white/15 hover:bg-white/5'}`}
    >
